...
The data was first processed in openrefine to enable georeferencing



```{r}
csv_files <- list.files(here::here("datasets", "kul-collections-taxidermi-birds-pas", "data", "interim"), pattern = "*.txt$", full.names = TRUE)

```

```{r}
data <- lapply(csv_files, read_delim) %>% bind_rows()
```
...
input_interim %>% head(n = 5)
```
# Save to csv for Openrefine actions

Save to CSV:

```{r}
write_csv(input_interim, here::here("datasets", "kul-collections-taxidermi-birds-pas", "data", "interim", "occurrence_interim.csv"), na = "")
```
...
No georeferencing possible here
...
```{r}
occurrence %<>% mutate(institutionCode = "KU Leuven") # e.g. "INBO"
```
### collectionCode
...
```{r}
occurrence %<>% mutate(basisOfRecord = "preservedSpecimen")
```
...
```{r}
occurrence %<>% rename(occurrenceID = `dwc:catalogNumber` ) %>%
