...
```{r}
occurrence %<>% mutate(scientificName = `dwc:scientificName`)
```
...
```{r}
occurrence %<>% mutate(taxonRank = `dwc:taxonRank`)
```
...
```{r}
occurrence %<>% mutate(occurrenceStatus = "present")
...
```{r}
colnames(occurrence) <- str_remove(colnames(distribution), "dwc_")
```
...
```{r}
write_csv(occurrence, here::here("datasets", "kul-collections-taxidermi-birds-pas", "data", "processed", "occurrence_geo.csv"), na = "")
```
